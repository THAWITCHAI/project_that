...
export default function TypeCars({ }: Props) {
    ...
    const handleDelete = async (id: number) => {
        const response = await fetch(`http://localhost:8000/type-car/${id}`, {
            method: 'DELETE',
        })
        if (response.ok) {
    ...
    const handleUpdate = async (id: number) => {
        const res = await fetch(`http://localhost:8000/type-car/${id}`, {
            method: 'PUT',
            headers: {
        ...
        toast.success('อัพเดทข้อมูลประเภทรถสำเร็จ', {
            action: {
                label: 'ปิด'
            }
        })
                ...
                <Table>
                    ...
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className='text-center'>รหัสประเภทรถ</TableHead>
