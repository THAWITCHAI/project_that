...
export default function Statuses({ }: Props) {
    ...
    const handleDelete = async (id: number) => {
        const response = await fetch(`http://localhost:8000/role-user/${id}`, {
            method: 'DELETE',
        })
        if (response.ok) {
            toast.success('ลบข้อมูลประเภทรถสำเร็จ', {
                action: {
                    label: 'ปิด'
                }
            })
    ...
    const handleUpdate = async (id: number) => {
        const res = await fetch(`http://localhost:8000/role-user/${id}`, {
            method: 'PUT',
            headers: {
        ...
        toast.success('อัพเดทข้อมูลบทบาทหน้าที่สำเร็จ', {
            action: {
                label: 'ปิด'
            }
        })
    ...
    return (
        <div className='w-full h-full flex flex-col justify-start gap-4 items-center'>
            <h1 className='w-full'>ข้อมูลบทบาทหน้าที่</h1>
                ...
                <Table>
                    ...
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className='text-center'>รหัสบทบาทหน้าที่</TableHead>
                            ...
                            <TableRow key={index}>
                                ...
                                <TableCell className='flex justify-end w-fit items-center gap-2'>
                                    ...
                                    <Sheet>
                                        ...
                                        <SheetContent>
                                            <SheetHeader>
                                                <SheetTitle>แก้ไขข้อมูลประเภทรถ</SheetTitle>
                                                <SheetDescription>
                                                    เปลี่ยนแปลงได้แค่ชื่อของข้อมูลเท่านั้น
                                                </SheetDescription>
                                                <Input type='text' placeholder='id' defaultValue={item.id} disabled />
                                                <Input onChange={(e) => setName(e.target.value)} className='border-green-500' type='text' defaultValue={item.name} placeholder='ชื่อประเภทรถ' />
